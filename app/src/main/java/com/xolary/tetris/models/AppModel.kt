...
class AppModel {
    ...
    // Перемещение разрешённого блока
    private fun moveValid(position: Point, frameNumber: Int?): Boolean {
        val shape: Array<ByteArray>? = currentBlock?.getShape(frameNumber as Int)
        return validTransaction(position, shape as Array<ByteArray>)
    }
    ...
    // Проверка на незаполненность поля и возможности перемещения блока
    ...
    // Сканирование строк на заполненность ячеек
    ...
    // Очищение строки в случае полного заполнения
    ...
    // Сохранение состояния всех ячеек
